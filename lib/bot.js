...
class Bot {
  ...
  disconnect() {
    logger.debug('Disconnecting from IRC and Discord');
    ...
    this.discord.on('message', (message) => {
      ...
      if (message.content.toLowerCase() == '/users') {
        ...
        if (this.channelUsers[ircChannel]) {
          ...
          logger.default.warn(`No channelUsers found for ${ircChannel} when /names requested`);
  ...
  sendToDiscord(author, channel, text) {
    ...
    // Check for MMX loli content
    if (author.toLowerCase().includes('mmx') && (text.includes('yande.re') || text.includes('twitter.com') || text.includes('lolibooru') || text.includes('4cdn.org') || text.includes('4chan.org') || text.includes('paheal.net') || text.includes('4cdn.org') || text.includes('twimg.com'))) {
      return;
    }
